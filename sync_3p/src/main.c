...
K_SEM_DEFINE(threadC_sem, 0, 1); /* starts off "not available" */
...
void threadB(void *dummy1, void *dummy2, void *dummy3)
{
	...
	helloLoop(__func__, &threadB_sem, &threadC_sem);
}
...
void threadC(void *dummy1, void *dummy2, void *dummy3)
{
	...
	/* invoke routine to ping-pong hello messages with threadA */
	helloLoop(__func__, &threadC_sem, &threadA_sem);
}
...
int main(void)
{
	k_thread_create(&threadA_data, threadA_stack_area,
			K_THREAD_STACK_SIZEOF(threadA_stack_area), threadA, NULL, NULL, NULL,
	...
	k_thread_create(&threadB_data, threadB_stack_area,
			K_THREAD_STACK_SIZEOF(threadB_stack_area), threadB, NULL, NULL, NULL,
			PRIORITY, 0, K_FOREVER);
	...
	k_thread_create(&threadC_data, threadC_stack_area,
			K_THREAD_STACK_SIZEOF(threadC_stack_area), threadC, NULL, NULL, NULL,
			PRIORITY, 0, K_FOREVER);
	k_thread_name_set(&threadC_data, "thread_c");

#if PIN_THREADS
	if (arch_num_cpus() > 2) {
		k_thread_cpu_pin(&threadC_data, 2);
	} else {
		k_thread_cpu_pin(&threadC_data, 0);
	}
#endif
